...
bool i2c_write(I2C_TypeDef* i2c, unsigned char device_address, unsigned char* data, unsigned char len)
{
    ...
    // unsigned char device_address, unsigned char* data, unsigned char len
...
uint8_t i2c_write_reg(uint8_t addr, uint8_t reg, uint8_t *buf, uint16_t len)
{
    // printf("i2c start\n");
    uint8_t regi[1] = {reg};
    if (!i2c_write(I2C1, addr, regi, 1)) {
        // printf("failure\n");
        return 1;
    }

    if(!i2c_write(I2C1, addr, buf, len)){
        return 1;
    }
    // printf("i2c end\n");
    return 0;
